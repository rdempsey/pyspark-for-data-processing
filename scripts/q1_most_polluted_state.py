"""
Get the state with the largest measured quantity of each potential
pollutant.

Output folder: q1_most_polluted_state
"""
...
"""
grouped = df.groupBy('parameter_name', 'state_name').agg(F.sum('arithmetic_mean').alias('arithmetic_mean'))
...
joined.write.option("header", "true").csv("/tmp/data/q1_most_polluted_state")
